...
| `subagent_interrupt` | Interrupt a running child without deleting its session |
...
- `/agents` — focus the `pi-agents` window
- `/agents focus [name]` — focus the window or a named child
- `/agents message <name> <message>` — steer or resume a child
- `/agents stop <name>` — interrupt a running child
- `/agents close-window` — close the owned agent window
- `Ctrl+Shift+A` — focus the agent window
